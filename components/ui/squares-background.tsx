...
export function Squares({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      gradient.addColorStop(1, baseBackground)
      ...
      if (excludeSelectors && excludeSelectors.length > 0) {
        ...
        excludeSelectors.forEach((selector) => {
          document.querySelectorAll(selector).forEach((el) => {
            ...
            const w = Math.min(canvas.width, rect.width)
            const h = Math.min(canvas.height, rect.height)
            if (w > 0 && h > 0) {
