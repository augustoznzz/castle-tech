...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // yNorm: 0.0 at top, 1.0 at bottom
  float yNorm = coord.y / iResolution.y;
  // Ensure strong effect starts from the very top and fades across entire height
  float verticalFade = 1.0 - smoothstep(0.0, 1.0, yNorm);

  // Brighter near the top, dimmer near the bottom
  float brightness = 1.0 - yNorm;
  fragColor.x *= 0.1 + brightness * 0.01;
  fragColor.y *= 0.3 + brightness * 0.01;
  fragColor.z *= 0.5 + brightness * 0.01;
...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const updatePlacement = () => {
        ...
        // Force perfect top-center anchor and straight down direction so it originates at the very top
        uniforms.rayPos.value = [0.5 * w, -0.1 * h];
        uniforms.rayDir.value = [0.0, 1.0];
      };
  ...
  useEffect(() => {
    ...
    const w = wCSS * dpr;
    const h = hCSS * dpr;
    // Keep top-center origin and downward direction on prop changes
    u.rayPos.value = [0.5 * w, -0.1 * h];
    u.rayDir.value = [0.0, 1.0];
  }, [
    raysColor,
    raysSpeed,
    lightSpread,
